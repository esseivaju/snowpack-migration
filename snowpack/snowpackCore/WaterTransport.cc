                                            ...
                                            SurfaceFluxes& Sdata)
{
	...
	*/
	if (ql > Constants::eps2) { // Add Mass
		...
		EMS[nE-1].theta[AIR] = MAX(0., 1.0 - EMS[nE-1].theta[WATER] - EMS[nE-1].theta[WATER_PREF] - EMS[nE-1].theta[ICE] - EMS[nE-1].theta[SOIL]);
				...
				*/
				...
				if ((EMS[e].theta[WATER]+EMS[e].theta[WATER_PREF]) > ((e==nE-1)?(2.*Constants::eps):0.)) {
					...
					if (-dM >= M) {
						...
						const double dM_pref = MAX(-EMS[e].theta[WATER_PREF], dM/(Constants::density_water*L0));
		...
		if (iwatertransportmodel_snow != RICHARDSEQUATION && (iwatertransportmodel_soil != RICHARDSEQUATION || nE>Xdata.SoilNode)) {
			for (size_t eUpper = nE-1, eLower = nE-2; eUpper >= 1; eUpper--, eLower-- ) {
				...
				if (eUpper == nE-1 && (W_upper > 0.0 && W_upper <= Wres)) {
					...
					EMS[eUpper].theta[AIR] = MAX(0., 1. - EMS[eUpper].theta[WATER] - EMS[eUpper].theta[WATER_PREF] - EMS[eUpper].theta[ICE] - EMS[eUpper].theta[SOIL]);
