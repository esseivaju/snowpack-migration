...
/************************************************************
 * StabilityAlgorithms static class                                           *
 ************************************************************/
/**
 ...
 * @param Xdata
 */
void StabilityAlgorithms::classifyStability_Fierz(const double& Swl_ssi, const size_t& Swl_lemon, const double& Swl_Sk38, SnowStation& Xdata)
{
...
double Stability::getHandHardnessMONTI(const ElementData& Edata)
{
	...
	if ( (Edata.mk%100) < 20 ) { // all types except MFcr (hardness 5)
		int F1, F2, F3; // grain shape
		typeToCode(&F1, &F2, &F3, Edata.type); // Decompose type in its constituents
		const double hardness_F1 = StabilityAlgorithms::getHandHardnessMONTI(F1, Edata.Rho, Edata.theta[WATER], hoar_density_buried);
		const double hardness_F2 = StabilityAlgorithms::getHandHardnessMONTI(F2, Edata.Rho, Edata.theta[WATER], hoar_density_buried);
...
void Stability::checkStability(const CurrentMeteo& Mdata, SnowStation& Xdata)
{
	...
	switch (Stability::prof_classi) {
		...
		case 1:
			...
			break;
		case 2:
			StabilityAlgorithms::classifyStability_Fierz(Swl_ssi, Swl_lemon, Swl_Sk38, Xdata);
			break;
		case 3:
